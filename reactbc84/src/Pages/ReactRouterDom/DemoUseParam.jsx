...
/*
    ...
    ∙ []: gọi làm 1 lần sau khi giao diện load
    ∙ [param]: Gọi khi param thay đổi (ứng dụng cho các trang detail hoặc search product)
    ∙ return: xóa các hàm chạy ngầm trong component (VD: api socket hay các hàm setTimeout, setInterval)
*/
const DemoUseParam = () => {
    ...
    const getProductDetailById = async () => {
        try {
            ...
            //Sau khi lấy dữ liệu đưa vào state
    ...
    }, [params.id]); // trường hợp 2 của useEffect, param thay đổi thì gọi lại api
                ...
                <div className="row">
                    ...
                    <div className="col-1 d-flex flex-column align-items-center gap-2">
                        {prodDetail.detaildetailedImages?.map((item, index) => {
                            return <div className="border" key={index}>
                                <img src={prodDetail.image} className="img-fluid rounded" alt="thumb1" key={index} style={{ transform: item }} />
                            </div>
                    ...
                    <div className="col-6">
                        <div className="bg-light d-flex align-items-center justify-content-center" style={{ height: '420px', position: 'relative' }}>
                            <img src={prodDetail.image} className="img-fluid" alt="main" />
                    ...
                    {/* Product info */}
                    <div className="col-5">
                        <h6 className="mb-1">{prodDetail.name?.toUpperCase()}</h6>
                        <div className="text-muted small mb-1">{prodDetail.shortDescription}</div>
                        <div className="mb-3 fw-bold">{prodDetail.price}$</div>
                        {/* Color options */}
                        <div className="d-flex gap-2 mb-3">
                            <img src="https://dummyimage.com/70x30/f00e38" className="img-thumbnail border-2 border-dark" alt="color1" />
                            <img src="https://dummyimage.com/70x30/09eb8d" className="img-thumbnail" alt="color2" />
                            <img src="https://dummyimage.com/70x30/cccccc" className="img-thumbnail" alt="color3" />
                        </div>
                        {/* Size selection */}
